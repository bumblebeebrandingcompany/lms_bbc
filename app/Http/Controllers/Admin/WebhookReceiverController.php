    ...
    */
    public function storeNewLead(Request $request)
    {
        try {
            ...
            $details['email'] = $req_data['lead']['email'] ?? '';
            $details['phone'] = $req_data['lead']['phone'] ?? '';
            $details['additional_email'] = $req_data['payload']['secondary_emails'][0]?? '';
            $details['secondary_phone'] = $req_data['payload']['secondary_phones'][0] ?? '';
            ...
            } else if(!empty($campaign_data['s']) && !in_array($campaign_data['s'], ['channel_partner'])) {
            ...
            $details['sell_do_stage'] = $req_data['payload']['stage'] ?? '';
            $details['sell_do_status'] = $req_data['payload']['status'] ?? '';
